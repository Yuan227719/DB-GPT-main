...
class VectorStoreBase(IndexStoreBase, ABC):
    ...
    def get_all_metadata(
        ...
        Used by DBSummaryClient to compare table names already stored in the
        vector store against the live database schema, so we only refresh
        the vector store when tables were actually added/removed (instead
        of unconditionally re-running DESCRIBE on every restart).

        Args:
            filters: Optional metadata filters (e.g. ``part == "table"``).

        Returns:
            List of metadata dicts, one per matching chunk.
        """
        raise NotImplementedError
