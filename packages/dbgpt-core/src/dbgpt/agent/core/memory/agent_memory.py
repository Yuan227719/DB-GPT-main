...
class AgentMemory(Memory[AgentMemoryFragment]):
    ...
    两者的关系：
    - ShortTermMemory 是进程内存，重启丢失；但 build() 时会从 gpts_messages 表恢复
    - gpts_memory 是持久化的，通过 REACT_AGENT_MEMORY_CACHE[conv_id] 跨请求复用
    - structure_clone() 时只克隆 ShortTermMemory，gpts_memory 是共享的
    """
