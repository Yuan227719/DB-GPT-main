    ...
    def embed_documents(self, texts: List[str]) -> List[List[float]]:
        ...
        Returns:
            Embedded texts as List[List[float]], where each inner List[float]
                corresponds to a single input text.
        """
        # Call OpenAI Embedding API.  Use a (connect, read) tuple timeout so
        # that a server that accepts the TCP connection but then never sends
        # any response bytes cannot hang the worker forever (a plain int
        # timeout only guards the read phase and can still get stuck on some
        # proxies / SSL handshakes).  Retry a few times with exponential
        # back-off because remote embedding endpoints occasionally drop a
        # request without responding at all.
        ...
        # connect timeout 10s, read timeout = self.timeout (default 60s)
        timeout_tuple = (10, self.timeout)
        ...
        for attempt in range(3):
            try:
                ...
                last_err = e
                logger.warning(
                    f"OpenAPIEmbeddings request failed (attempt {attempt + 1}/3, "
        ...
        raise RuntimeError(
            f"OpenAPIEmbeddings request failed after 3 attempts "
            f"(model={self.model_name}, texts={len(texts)}): {last_err}"
        )
