...
class Service(
    ...
    def refresh(self, datasource_id: str) -> bool:
        ...
        # async embedding. ``force=True`` makes db_summary_embedding delete
        # the existing profile first and then re-embed, all under the same
        # per-db indexing lock. This avoids the race where a standalone
        # delete drops the chroma collection while an in-flight embedding
        # flow is still writing chunks to it, producing
        # "Collection <uuid> does not exist" errors for every chunk.
        def _refresh_job():
            self._db_summary_client.db_summary_embedding(
                db_config.db_name, db_config.db_type, force=True
            )

        executor = self._system_app.get_component(
            ComponentType.EXECUTOR_DEFAULT, ExecutorFactory
        ).create()  # type: ignore
        executor.submit(_refresh_job)
        return True
