...
class StorageManager(BaseComponent):
    ...
    def invalidate_store(self, index_name: str) -> None:
        """Drop the cached VectorStoreBase for ``index_name``.

        Must be called after ``delete_vector_name`` so the next
        ``create_vector_store`` returns a fresh store whose internal
        ``self._collection`` reference points at a live collection instead
        of the now-deleted one. Without this, post-delete writes go through
        the stale cached store and fail with
        ``Collection <uuid> does not exist``.
        """
        with self._cache_lock:
            self._store_cache.pop(index_name, None)
