...
class VectorStoreConnector:
    ...
    def get_all_metadata(self, filters: Optional[MetadataFilters] = None):
        """Return metadata of all chunks matching ``filters``.

        Delegates to the underlying vector store (e.g. ChromaStore) so the
        DBSummaryClient can read the table names already stored in the
        vector store and compare against the live DB schema.
        """
        if not hasattr(self.client, "get_all_metadata"):
            raise NotImplementedError(
                f"Vector store {type(self.client).__name__} does not support "
                "get_all_metadata"
            )
        return self.client.get_all_metadata(filters)
