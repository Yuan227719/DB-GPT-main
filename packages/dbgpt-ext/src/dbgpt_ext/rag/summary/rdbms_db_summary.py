...
class RdbmsSummary(DBSummary):
    ...
    def __init__(
        ...
        # 【性能优化】不在构造函数里遍历所有表跑 _parse_table_summary
        # 原因：init_db_profile 流程只用 self.db（connector），从不读 table_info_summaries。
        # 构造时遍历会导致每张表都跑一次 DESCRIBE（get_columns），
        # 而 DBSchemaAssembler 后面会再跑一次，造成 DESCRIBE 重复调用。
        # 改为懒加载：只在 table_summaries() 被调用时才计算。
        self.table_info_summaries = None
    ...
    def table_summaries(self):
        ...
        懒加载：首次调用时遍历所有表计算，后续调用复用缓存结果。
        init_db_profile 流程不调用此方法，因此不会触发 DESCRIBE。
        """
        if self.table_info_summaries is None:
